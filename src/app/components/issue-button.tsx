...
export default function IssueButton({
  ...
  const { trigger, isMutating } = useSWRMutation(
    "/api/generate-issue-session",
    (url, { arg }: { arg: any }) =>
      fetcher<{ session: { url: string } }>(url, {
        method: "POST",
  ...
  const onIssue = async () => {
    const result = await trigger({ claim, dataModel, image });
  ...
  return (
    <>
      <button
        type="button"
        className={["gtw-btn", (!isEnabled || isMutating) && "!opacity-50"]
          .filter(Boolean)
          .join(" ")}
        onClick={onIssue}
